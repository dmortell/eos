<script lang="ts" module>
	/** Minimal view-state contract — tool-specific ViewState types (racks, elevations)
	 *  satisfy this structurally; extra fields (scale, bottom, …) pass through untouched. */
	export interface PanZoomViewState {
		x: number
		y: number
		zoom: number
		grid: number
...
<script lang="ts">
	/**
	 * Generalized pan/zoom canvas — extracted from racks/parts/Canvas.svelte
	 * (which keeps its own copy until the racks tool migrates). Mouse: middle/right
	 * drag pans, ctrl/alt/meta+wheel (or right-button+wheel) zooms about the
	 * cursor with eased rAF. Touch: two-finger pan+pinch (instant), optional
	 * one-finger background pan with tap detection.
	 */
