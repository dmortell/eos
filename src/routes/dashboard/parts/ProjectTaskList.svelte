<script lang="ts">
	...
	let { tasks = [], projects = [] }: { tasks: Task[]; projects: Project[] } = $props();

	let rows = $derived.by(() => {
		const nameById = new Map(projects.map((p) => [p.id, p.name]));
		const grouped = new Map<string, { projectId: string; name: string; pending: number; inProgress: number; review: number; done: number }>();
		for (const task of tasks) {
			const key = task.projectId || 'unknown';
			if (!grouped.has(key)) {
				grouped.set(key, {
					projectId: key,
					name: nameById.get(key) || task.projectName || key,
					pending: 0,
					inProgress: 0,
					review: 0,
					done: 0
...
<div class="border rounded-md bg-white overflow-x-auto">
	<table class="w-full text-xs">
		<thead class="bg-gray-50 text-gray-600">
			<tr>
				<th class="text-left px-2 py-1">Project</th>
