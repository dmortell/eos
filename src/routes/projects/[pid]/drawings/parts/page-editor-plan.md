...
- **`outlets` viewport.** `OutletsViewport.svelte` renders the floorplan PDF underlay + SVG overlay with outlet dots (color-keyed by usage) and trunk/route polylines. Calibration-aware: real-world mm → PDF-px via `files/{id}.pages[n].origin + scaleFactor`. `showOutlets` / `showTrunks` toggles on the source gate the overlay groups.
...
- **Secondary route per-cable pricing** — route labels show total length only; add cable-count / fill-rate style annotations once the route data gains per-cable metadata.
- **Full AnnotationOverlay on album tiles** — album tiles now show an annotation-count badge, but don't render the actual pins. Rendering full overlays at tile scale would be cluttered; a click-through to single mode is probably the right UX.

### Newly shipped this round

- **Racks plan-view — rack appearance** — [RackPlanRenderer.svelte](../../../racks/parts/RackPlanRenderer.svelte) swapped saturated solid-fill racks for architectural styling: light pastel fill + 1.5 px colored border + dark text per type (emerald for racks, slate with diagonal-stripe pattern for cabinets, rose-striped for VCMs, amber for desks/shelves). Labels get a white text-shadow so they stay readable on any pattern.
- **Racks plan-view — drawable-area indicator** — dashed gray rect at `0,0 → PLAN_CANVAS_MM × _MM` plus a crosshair + `0,0` text marker. Users can now see where the drawable area starts and stops, solving the "nothing happens when I click top-left of origin" mystery. `vector-effect="non-scaling-stroke"` keeps the outline crisp at any canvas zoom.
- **Racks plan-view — wall caps + rect thickness** — wall strokes changed from `stroke-linecap="round"` to `"butt"` so wall ends are square like real drawings. Rect stroke-width now honours `obj.thicknessMm` (default 30) instead of the hard-coded 30/40 pair; selection still bumps to `max(thick, 40)` for visibility.
- **Racks plan-view — draggable objects + endpoints** — new `onupdateroomobject` callback + `startObjDrag` state machine. Walls / doors / rects can be dragged by body (whole translate); when a wall is selected, endpoint handles appear at each end for independent repositioning; when a rect is selected, NW/NE/SW/SE handles enable corner-anchored resize with opposite-corner clamping. All drags snap to the 100 mm grid on mouseup.
- **Survey album annotation badges** — [SurveyViewport.svelte](../viewports/SurveyViewport.svelte) tiles now show an amber count badge in the top-right when a photo has annotations so users can spot annotated photos at a glance without clicking through.
- **Page editor viewport list** — [drawings/pages/[pageId]/+page.svelte](../../../drawings/pages/[pageId]/+page.svelte) sidebar grew a collapsible list of every viewport on the page (kind slug + label + pinned-rev badge). Click to select, double-click to jump to the source. Complex pages (viewports behind title block, tiny text blocks, stacked frames) become navigable without hunting on canvas.
- **Mixed-paper-size printing** — [packages/[pkgId]/print/+page.svelte](../../../packages/[pkgId]/print/+page.svelte) computes `uniquePaperConfigs` from the package's resolved pages, emits one `@page pkg_<size>_<orientation> { size: W mm H mm }` rule per unique combo, then tags each `.sheet-wrap` with a matching `.sheet-size-<size>-<orientation>` class carrying `page: pkg_<size>_<orientation>`. Chrome honours the CSS `page` property at page-break boundaries, so a package mixing A3-landscape + A4-portrait now prints each sheet at its correct paper size + orientation in one job. Previously the first page's size got forced onto the entire job.
- **Secondary route shapes** — [OutletsViewport.svelte](../viewports/OutletsViewport.svelte) styles outlet→rack `routes` to match trunks: polyline body with rounded joins, waypoint bullets at intermediate nodes, and an optional length pill at the midpoint. The trunk renderer handles true trunks; routes keep the simpler polyline geometry since they carry no shape catalog.
- **Keyboard shortcuts for page editor** — [drawings/pages/[pageId]/+page.svelte](../../../drawings/pages/[pageId]/+page.svelte) wires `onWindowKeyDown` to `<svelte:window>`. With a viewport selected: arrow keys nudge 1 mm (10 mm with shift), Delete/Backspace removes, Ctrl/⌘-D duplicates (offset 10 mm down-right, auto-selected), Escape deselects. Handler is gated to skip when an `<input>` / `<textarea>` / `contentEditable` element has focus so sidebar typing isn't hijacked. The viewport-frame hint line under a selected viewport now lists the shortcuts.
