...
export async function exportToExcel(racks: RackData[], zone: ZoneConfig) {
	const wb = new ExcelJS.Workbook()
	const ff = String(zone.floor).padStart(2, '0')

	for (const rack of racks) {
		const sheetName = `${rack.frame.name}`.substring(0, 31)
		const ws = wb.addWorksheet(sheetName)

		// Title
		ws.mergeCells('A1:Z1')
		const titleCell = ws.getCell('A1')
		titleCell.value = `Floor ${ff} · Zone ${zone.zone} · ${rack.frame.name} (${rack.frame.totalRU}U)`
		titleCell.font = { bold: true, size: 14 }
		titleCell.alignment = { horizontal: 'left' }

		// Column header row (port positions 1-24)
		const headerRow = 3
		ws.getCell(headerRow, 1).value = 'RU'
		ws.getCell(headerRow, 1).font = { bold: true, size: 8 }
		ws.getCell(headerRow, 1).alignment = { horizontal: 'center' }
		ws.getCell(headerRow, 2).value = 'Row'
		ws.getCell(headerRow, 2).font = { bold: true, size: 8 }

		for (let col = 0; col < 24; col++) {
			const cell = ws.getCell(headerRow, col + 3)
			cell.value = col + 1
		...
		// Set column widths
		ws.getColumn(1).width = 4   // RU
		ws.getColumn(2).width = 3   // Row (T/B)
	...
	a.href = url
	a.download = `PatchFrame_F${String(zone.floor).padStart(2, '0')}_Z${zone.zone}.xlsx`
	a.click()
