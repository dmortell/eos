<script lang="ts">
	...
	let { editor, tool = $bindable(), annEditor, vps = null, face = 'front', libraryOpen = false, racksOpen = false, layers = [], ondevices, onracks }: { editor: RacksEditor; tool: string; annEditor: AnnotationEditor; vps?: import('../../viewports.svelte').ViewportEditor | null; face?: RackFace; libraryOpen?: boolean; racksOpen?: boolean; layers?: import('../../layers/layers').LayerDef[]; ondevices?: () => void; onracks?: () => void } = $props()
	...
	$effect(() => {
		if (!vps) return
		vps.dxfExport = { label: `rack ${face}`, run: exportDxf }
