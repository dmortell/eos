...
|---|---|---|
| **Place** | A node of the building fabric: building › floor › zone › room › row. Pages owns its own copy; it's seeded once from project data and edited only in Pages. | `places` field on the Pages project doc (§2.1) |
...
| **Annotation** | 2D markup. It lives **in the model**, tagged with the view it belongs to, and only ever appears through a frame. Nothing is drawn directly on the paper. | model `ents` |

Floor nodes in the tree open that floor's model (a model tab). Model tabs are for editing geometry, not for
issuing. Models are reached through their place, not listed as drawings.

## 2. Storage (Firestore)

### 2.1 Pages project doc — `projects/{pid}/pages/project` (proposed path)
- `places`: the Pages place tree. Each node has a **stable id** (nanoid), `kind` ('building' | 'floor' | 'zone' |
...
- Floor levels and heights are stored on the models. The building model is the source of truth for levels
  (slab, raised floor, ceiling).
...
- **Re-issuing after edits:** if a sheet or one of its models was edited after the last save or issue, issuing
  ...
  number first.
- ◧ **To settle when building this:** an issue must reference a version that has a stored copy. Either store
  a copy for every version that gets issued (minor ones included), or make issuing require a major version.
  The plan assumes the first: the copy is taken when the version is first issued.
...
1. **Schema + stores:** shared types (`Place`, `SheetDoc` content, `Frame`, the `Model` additions); Firestore
   services for the Pages project doc, sheets (registry `toolType: 'pages'`) and models, with debounced
