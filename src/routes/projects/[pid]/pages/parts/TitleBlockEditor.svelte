<script lang="ts">
	...
	import { TB_AUTO, DEFAULT_TITLE_BLOCK, type TitleBlockTemplate, type TbField, type TbAutoKey } from '../titleBlock'
	...
	const set = (patch: Partial<TitleBlockTemplate>) => onchange({ logo: t.logo ?? '', fields: t.fields.map((f) => ({ ...f })), ...patch })
