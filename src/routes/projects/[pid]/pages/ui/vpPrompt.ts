...
export function toolPrompt(a: PromptArgs): string {
	...
	switch (tool) {
		case 'Select': return a.graphSelected ? 'Drag a node to reshape (a node shared with another run brings it along — Alt-drag pulls it off) · click a segment to select just it · Ctrl-drag a node to branch · double-click a segment to add a node' : 'Click an element'
