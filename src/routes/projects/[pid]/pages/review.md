...
| B17 (update) | — | **Superseded — deferred to X4** (Dave's call, 2026-09-23, relayed via eos-07, made alongside the R8-lite decision). The row above's "R1/R9" plan is no longer the target: R9 landed the prop-explosion half (the `Workspace` object, `4d7f8e3`) but the module-singleton leak itself (`models`/`layers`/`layerUI`/`imgEdit`/`docs`/`viewState`/`selStore` still shared across a project→project client-side navigation) is explicitly NOT being chased standalone — it waits for **X4** (persistence pattern for §12), since real per-project storage/lifecycle will settle the same question anyway and patching it twice is wasted work. Not started. |
...
| R1 remainder — Viewport split to target | (this commit) | **Done — ≤ 600 target met: `Viewport.svelte` 1448 → 90 lines.** The component is now a shell (props, DOM event wiring, SVG skeleton). `ui/vpView.svelte.ts` `VpView` is the view model: props + defaults, mapping, pan/zoom, ctx, layers + VP Freeze, Selection, picking/`pickAt`, grips, underlay fit. `ui/vpInteraction.svelte.ts` `VpInteraction` is the pointer/key state machine (draft, every drag, marquee, text edit, image calibration, status). `ui/vpPrompt.ts` holds the pure prompt/status text (+ 4 tests). Markup moved to `render/VpMarks` (guides + sections, under entities), `render/VpOverlays` (grips/snap/marquee/preview, above) and `render/VpWidgets` (section toolbar, text editor, scale entry). Types moved to `ui/vpTypes.ts`, and the `+page`/`PaperPage`/`types.ts` imports were updated. No prop/API change for callers. Gate clean (367 tests). **Live 2026-09-23 on 5173 (Hibiya 30F model + the Plan and Elevations Test sheet), zero console errors:** tree restore + floorplan zoom-to-extents; wheel zoom 20 / 0.05 limits; ACAD rect by 2 clicks (prompt progression + preview); edge-click select (4 grips); body drag (grid-snapped); BR grip resize (TL fixed); Ctrl+Z; L→R marquee; Delete + undo; Text place → dbl-click editor → Ctrl+Enter commit; guide preview/place/drag; section draw → selected (4 arrows, 4 grips, toolbar, status) → border move → toolbar delete; Wall polyline + Enter → node grip drag → dbl-click insert (3 → 4 nodes); iso orbit; Rear elevation; paper frame dbl-click activate → draw → select → Esc ×2 (clear, deactivate). |
...
| **B31** (new, Dave 2026-09-23; eos-07 implementing) | — | **A model tab behaves like a frame on paper, not a model editor.** (1) It must be double-clicked to activate before drawing (tool strip dimmed until then; Esc / a dblclick on the pane exits); (2) two zoom layers: the pane's canvas transform wraps the Viewport's own view, so while not active (or with "Pan content" off) right-drag/wheel move and scale the whole editor box instead of the drawing — not an infinite canvas; (3) it draws on white paper. **Fix:** a model-layout pane (a non-sheet tab, or a sheet in Full-size) is always active, has no canvas transform (pan/zoom always act on the Viewport's own view, so "Pan content" matters only on sheets), and renders AutoCAD-style dark model space (low-contrast ink/layer colours lifted for the dark background). |
| **R5 — one layer model** (+ **B16 closed**) | (this commit; eos-07) | **Done.** `3dview/types.ts Layer` is the only layer type (+ `group`/`swatch`/`dash`); each model's `layers` list holds its object layers (group undefined = "Model") AND its Background / annotation / outlet / trunk / architectural layers (`mock/layers.ts layerStack`), so entities and objects resolve hide / lock / colour / draw order against the SAME per-model list. `layers.svelte.ts` is now helpers over a list + the two non-model pieces of UI state (active layer, View Presets); `PLayer` and the global list are gone. LayersPanel = one grouped list of the focused model (no separate Model section); PropertiesPanel's entity and object Layer dropdowns read the same list; `place.resolveLayer` falls back to the first OBJECT layer (never a Background one); a new entity lands on the active layer if the model has it, else Annotations (`activeLayerIn`). Seed presets list the demo object-layer ids so "High Level Outlets" doesn't hide walls. `layers.svelte.test.ts` (7) + a resolveLayer case. **Deferred, noted in the store's header:** paper-space layers for a sheet (no paper-space annotations exist yet) and per-viewport overrides (K9 per-frame frozen layers). **Behaviour note:** layer visibility now lives in the model, so it rides model history snapshots — an undo can restore an older visibility (was already true of the B16 interim model layers). |
| **R7 — one projection path** | (this commit; eos-07) | **Done.** `projection.viewMap(dir, cx, cy, ground, yaw, pitch, isoBox)` → `{ toDraw(p3), uvToDraw(u,v), xform }` is the ONE engine/model → drawing mapping: Model3d renders through its `xform`; `hit.viewMapOf(ctx)` feeds `graphNodeDraw`, `prismRect` (elevation), `prismOutline`, `graphHit`'s face test and `hitModelIso`; the Viewport's `isoGround` uses it too. The hand-written copies (Model3d's per-dir `ox`, graphHit's `ox`, hitModelIso's / isoGround's iso centring, elevU in prismOutline/graphNodeDraw) are gone. **`hitModel` for prisms = point in (or within tol of) the DRAWN `project()` outline** (`prismOutline` for every prism), so rotation, tilt and odd edge counts come for free — the rot-branch / tilted-branch / AABB-branch are deleted. **Behaviour changes (intended):** a plan prism rotated about z no longer picks at its unrotated AABB corners; an elevation prism rotated about z picks its full drawn silhouette (was the unrotated w/d face); a hexagon misses its bbox corners. `prismRect` stays the EDIT box (unrotated x/y/w/d/z/h) the grips sit on. `elevU`/`elevUInv` (entities + the inverse edit maths) are pinned equal to `viewMap` by test rather than rewritten. `ui/viewMap.test.ts` (9). Live: desk pick in plan, device pick in the Rack A elevation and in 3D; no console errors. **Not done (P2/P4):** hover still runs three hit passes; iso faces are still recomputed per click. |
| **P2 — hover** | (this commit; eos-07) | **Done.** The Select-tool hover cursor asks `pickAt` — the same cascade a press uses (model grip, section grip, entity grip / body, guide, section, model object) — so hover and press always agree, and runs at most ONCE per animation frame on the latest pointer position (was `hit` + `hitModel` + `hitGuide` on every pointermove). Live: 'move' over a desk and a wall, 'default' just outside. Note: `requestAnimationFrame` never fires in a hidden tab, so automation must stub it (a visible tab is unaffected). |
| **P3 — short term** | (this commit; eos-07) | **Done (short term; command history is the long-term fix, todo §10).** Finding first: the review's "2 MB photo × 100 steps = 200 MB" doesn't happen — `$state.snapshot` shares string values, it doesn't copy them. The real costs were (1) every pointer move of a drag snapshotting ALL models (`updateStep` per `modeledit`) — now a gesture only flags `gestureDirty` and its final state is folded ONCE at gesture end (`flushGesture` also runs before undo / redo / jump); (2) `hist` being deep `$state`, so undo re-proxied whole snapshots — now `$state.raw`; (3) image bytes inside the model (they'd break Firestore's 1 MB doc limit at §12) — `imageStore.ts` interns a data-URL to an `img:<id>` key (render resolves it; the X7 `fileId` stand-in, session-only). Live: drag-resize a desk → undo restores → redo restores the FINAL size; an imported data-URL stores `img:…` and renders. `imageStore.test.ts` (4). |
| **P4 — iso pick faces** | (this commit; eos-07) | **Done.** `hit.isoPickFaces(ctx, ml)` projects every pickable face once (R7 `viewMapOf`, camera depth of 3 corners) and the Viewport keeps it in a `$derived` (recomputes on model / orbit / layer change only); a click runs `hitIsoFaces(faces, p)`. `hitModelIso` stays as the one-call wrapper. Live: a 3D click selects one object (its 6 faces go amber). |
...
**Carries B17's remainder** (Dave, 2026-09-23, §0a): the module-singleton leak across a
project→project client-side navigation (`models`/`layers`/`layerUI`/`imgEdit`/`docs`/`viewState`/
`selStore` all module-level `$state`) is deferred here rather than patched standalone — whatever
per-project store lifecycle X4 settles (a `{#key page.params.pid}` reset, per-instance ownership,
or the real Firestore-subscription teardown) should close B17 as a side effect, not a separate fix.
