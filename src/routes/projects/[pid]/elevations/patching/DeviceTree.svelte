<script lang="ts">
	/**
	 * Bench sidebar: room → rack → port-bearing device tree with search.
	 * Click a rack to add its board; click a device to add its rack's board
	 * scrolled to + highlighting that device (§13).
	 */
	import { Icon } from '$lib'
	import type { BenchEditor } from './bench.svelte'

	let { editor }: { editor: BenchEditor } = $props()
			...
			const racks = editor.racks
				...
				.map(rack => {
					const devices = editor.devices
						.filter(d => d.rackId === rack.id && (d.portCount ?? 0) > 0)
				...
				.filter(({ rack, devices }) => !q || devices.length > 0 || rack.label?.toLowerCase().includes(q))
			return { room, racks }
	...
	<div class="p-2 border-b border-gray-100">
		<div class="relative">
			...
			<input class="w-full h-6 pl-6 pr-2 border border-gray-200 rounded text-[11px]"
				placeholder="Search racks & devices…" bind:value={query} />
		</div>
	</div>
	<div class="flex-1 overflow-y-auto p-1 space-y-1.5">
		{#if tree.length === 0}
			<div class="p-2 text-[11px] text-gray-400">No racks on this floor{query ? ' match the search' : ''}.</div>
		{/if}
		{#each tree as { room, racks } (room)}
			<div>
				<div class="px-1.5 text-[10px] text-gray-400 uppercase tracking-wider font-medium">Room {room}</div>
				{#each racks as { rack, devices } (rack.id)}
					...
					<div class="mt-0.5">
						...
						{#each devices as d (d.id)}
							<button class="w-full flex items-center gap-1.5 pl-6 pr-1.5 py-px rounded text-left text-[11px] text-gray-500 hover:bg-gray-50"
								title="Show this device on the bench"
