<script lang="ts">
	...
	let inner = $derived(RACK_19IN_INNER / 2 * scale)
...
{:else}
	...
	{#each Array.from({ length: rack.heightU }) as _, i}
		...
		<!-- RU numbers -->
		<text x={cx - inner - 10} y={yBottom - 4} fill={overlaps.has(u) ? '#000' : '#999'} font-size={Math.max(6, 22 * scale)}
