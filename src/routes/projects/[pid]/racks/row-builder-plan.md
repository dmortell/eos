...
Viewport rotation — ViewportFrame.svelte. Content now wraps in a rotating container centred in the frame. At 90°/270° the inner box is sized with swapped dimensions (innerW = isQuarterRot ? hPx : wPx) and offset so rotation around centre lines up with the frame's bounds. Sidebar grew a 4-pill picker (0° / 90° / 180° / 270°) wired to updateViewportLive. Frame footprint + drag/resize handles stay axis-aligned — only the content rotates, so you can drop a vertical rack elevation on a landscape page without having to rotate your head.

Rack-elevation multi-row — RackElevationViewport.svelte. Layout algorithm changed: when source.rowId is set → single-row as before; when unset → iterate all rows in order, laying out each row's racks sequentially with a ROW_GAP_MM = 600 gap between rows. Rear-face derivation runs once over the full laid-out list so rear ordering is consistent across the whole room. Sidebar source picker has a new "Row id (empty = all rows)" input.

Rear-view wall mirroring — RackElevationRenderer.svelte. Computes rowEndXMm from whichever rack list is active, then in rear face mirrors the walls around that end-point: effLeftWallX = rowEndXMm - rightWallX, effRightWallX = rowEndXMm - leftWallX. Slab / label / wall positions all use effLeftWallX / effRightWallX. The drag handlers flip which stored wall field they target in rear view — the physical wall's identity is preserved, so dragging the (visually) left wall in rear view edits what's actually the rightWallX setting. Stale TODO comment removed.

Frame-detail full port-grid — FrameDetailViewport.svelte completely rewritten. Subscribes to frames/{frameDocId} + all four racks/{frameDocId}_R{A..D} docs, then runs the frames-tool pipeline (deriveFramesFromRacks → generatePortLabels → generateRacks) to get the full RackData[], filters to src.frameId, and mounts the real FrameDrawing.svelte read-only inside an overflow-hidden scaled container. Page gets the same port-grid visual authors see in the frames tool — copper-panel ports, high-level ports, device slots, opposite-face panel hatching — without reimplementing any of it. To support this I moved deriveFramesFromRacks + matchesFace out of Frames.svelte and into frames/parts/engine.ts so both call sites share one source of truth.

Plan doc's follow-up section reshuffled: the four done items moved to a new "Viewports — shipped" block, and the truly outstanding ones (trunk-shape fidelity, survey annotations, patching label resolution, visual title-block editor, mixed-paper-size printing, stale-badge precision) consolidated into "Viewports — still outstanding" + "Other known follow-ups".